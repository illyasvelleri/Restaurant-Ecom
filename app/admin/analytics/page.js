...
// app/admin/analytics/page.js → PREMIUM ADVANCED ANALYTICS 2025
// FONTS CHANGED TO STANDARD READABLE ADMIN PANEL STYLE (Inter + system fallback)
// ADDED: restaurant timezone from PublicSetting (no lines removed)
...
export default function AnalyticsPage() {
    ...
    // ── Original fetch logic — untouched + timezone added ──
    const fetchAnalytics = async () => {
        try {
            setLoading(true);
            ...
            if (settingsRes.ok) {
                ...
                setTimezone(s.timezone || "Asia/Riyadh"); // ← NEW: load trusted timezone
            }
        } catch (err) {
            toast.error("Failed to load analytics: " + err.message);
            console.error(err);
