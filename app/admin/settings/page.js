...
// app/admin/settings/page.js → FINAL 2025 MOBILE-FIRST LUXURY (LIKE YOUR SAMPLE)
// UPDATED: currency + timezone integrated into restaurantData & passed to components
...
// Full list of real-world timezones (official IANA — never miss any)
        ...
        if (settingsRes.ok) {
          ...
          if (data.restaurant) {
            setRestaurantData(prev => ({
              ...prev,
              ...data.restaurant,
              // ensure both fields always have values even if missing from DB
              currency: data.restaurant.currency || 'SAR',
              timezone: data.restaurant.timezone || 'Asia/Riyadh'
            }));
          }
  ...
  const handleSave = async () => {
    try {
      const res = await fetch('/api/admin/settings', {
        ...
        body: JSON.stringify({
          profile: profileData,
          restaurant: restaurantData,          // ← now includes currency + timezone
          notifications: notificationSettings,
          delivery: deliverySettings,
        }),
      });
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          ...
          {activeTab === 'restaurant' && (
            <RestaurantSettings 
              ...
              timezones={TIMEZONES} // ← pass full list to component
            />
          )}
          {activeTab === 'security' && <SecuritySettings />}
          {activeTab === 'notifications' && (
          ...
          {activeTab === 'payment' && (
            <PaymentSettings
              ...
              setCurrency={(newCurrency) =>
                setRestaurantData(prev => ({ ...prev, currency: newCurrency }))
              }
            />
          )}
          {activeTab === 'delivery' && (
...
// // app/admin/settings/page.js → FINAL 2025 MOBILE-FIRST LUXURY (LIKE YOUR SAMPLE)
// // UPDATED: currency integrated into restaurantData & passed to PaymentSettings
...
//     address: '',
//     website: '',
//     timezone: 'Asia/Riyadh',     // optional – can be removed if not used
//     currency: 'SAR'               // ← ADDED: currency field
//   });
...
//               ...prev,
//               ...data.restaurant,
//               // ensure currency always has a value even if missing from DB
//               currency: data.restaurant.currency || 'SAR'
//             }));
//           }
...
//         body: JSON.stringify({
//           profile: profileData,
//           restaurant: restaurantData,          // ← now includes currency
//           notifications: notificationSettings,
//           delivery: deliverySettings,
//         }),
//       });
