...
// app/user/combos/page.js — PRO LUXURY REDESIGN 2025 (DYNAMIC CURRENCY + TIMEZONE FROM DB)
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            {combos.map((combo, index) => (
              <ModernComboCard
                ...
                onAdd={addToCart}
                currency={currency}
                timezone={timezone} // ← NEW: pass timezone to card if needed later
                index={index}
              />
            ))}
          </div>
...
function ModernComboCard({ combo, onAdd, currency, timezone, index }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 30 }}
      animate={{ opacity: 1, y: 0 }}
