...
// app/user/cart/page.jsx — UPDATED: uses restaurant timezone from DB
...
function CartContent() {
  ...
  const [selectedItemIndex, setSelectedItemIndex] = useState(0);
  const [currency, setCurrency] = useState("SAR");
  const [timezone, setTimezone] = useState("Asia/Riyadh"); // ← NEW: restaurant timezone
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [aiRecommendations, setAiRecommendations] = useState([]);
  const [loadingRecommendations, setLoadingRecommendations] = useState(false);
    ...
    const loadCart = async () => {
      try {
        ...
        if (resSet.ok) {
          const data = await resSet.json();
          setCurrency(data.currency || "SAR");
          setTimezone(data.timezone || "Asia/Riyadh"); // ← NEW: load restaurant timezone
        }
      } catch (err) {
        console.error(err);
        toast.error("Failed to sync prices");
  ...
  // Load AI recommendations when cart changes — NOW TIMEZONE-AWARE
  useEffect(() => {
    if (cart.length > 0 && allProducts.length > 0) {
      loadAIRecommendations();
    }
  }, [cart, allProducts, timezone]); // ← added timezone dependency

  const loadAIRecommendations = async () => {
    setLoadingRecommendations(true);
    try {
      // NEW: Use correct timezone for timeOfDay & dayOfWeek
      const now = new Date();
      ...
      const res = await fetch("/api/user/ai-recommendations", {
        ...
        body: JSON.stringify({
          cartItems: cart,
          allProducts: allProducts,
          timeOfDay: parseInt(timeOfDay), // correct local hour
          dayOfWeek: dayOfWeek,           // correct local day name
          currency: currency,
          timezone: timezone              // optional: send for backend awareness
        })
      });
    ...
    try {
      const res = await fetch("/api/user/updateOrder", {
        ...
        body: JSON.stringify({
          ...
          totalAmount: parseFloat(total),
          timezone: timezone // ← optional: send timezone with order
        })
      });
    ...
    <>
      ...
      <div className="cart-page">
        ...
        <div className="cart-content">
          ...
          {/* EXISTING AI RECOMMENDATIONS SECTION – NOW TIMEZONE-AWARE */}
          <div className="ai-recommendations-section">
            <div className="ai-recommendations-header">
              <span className="ai-badge">
                <Sparkles size={12} />
            ...
            <p className="ai-recommendations-subtitle">
              {loadingRecommendations 
                ? "Analyzing your order..." 
                : `Based on your order and ${new Date().toLocaleString('en-US', { timeZone: timezone, hour: 'numeric', hour12: false }) < 12 ? "morning" : new Date().toLocaleString('en-US', { timeZone: timezone, hour: 'numeric', hour12: false }) < 17 ? "afternoon" : "evening"} preferences`
              }
            </p>
