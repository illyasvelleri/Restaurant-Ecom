...
//   );
// }
// app/user/checkout/page.js → PREMIUM REDESIGN 2025 (DYNAMIC CURRENCY + TIMEZONE FROM DB)
// ALL ORIGINAL LOGIC PRESERVED + TABLE BOOKING + TIMEZONE SUPPORT
...
function CheckoutContent() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        // Fetch restaurant details — now includes timezone
        const resDetails = await fetch("/api/restaurantDetails");
        if (resDetails.ok) {
          const data = await resDetails.json();
          setCurrency(data.currency || "SAR");
          setTimezone(data.timezone || "Asia/Riyadh"); // ← NEW: load correct timezone
          const wa = data.whatsapp?.replace(/\D/g, "") || "";
          if (wa && wa.length >= 10) setWhatsappNumber(wa);
        }
  ...
  const sendOrder = async () => {
    ...
    // ── NEW: Use correct restaurant timezone for order timestamp ──
    const now = new Date();
    const orderTime = now.toLocaleString("en-GB", {
      timeZone: timezone, // ← use DB timezone
      day: "2-digit", month: "short", year: "numeric",
      hour: "2-digit", minute: "2-digit", hour12: true
    });
    // -------------------------------------------------------------
    ...
    try {
      ...
      const res = await fetch("/api/user/saveOrders", {
        ...
        body: JSON.stringify({
          ...
          tableNumber: mode === "table" ? tableNumber : null,
          status: isEditMode ? "confirmed" : "pending",
          timezone: timezone // ← optional: save timezone with order
        })
      });
