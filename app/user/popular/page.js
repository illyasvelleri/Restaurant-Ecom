...
// app/user/popular/page.js — PREMIUM REDESIGN 2025 (DYNAMIC CURRENCY + TIMEZONE FROM DB)
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-12">
            {popularItems.map((item, index) => (
              <motion.div
                ...
                transition={{ duration: 0.6, delay: index * 0.1 }}
              >
                <PremiumFoodCard dish={item} onAdd={addToCart} currency={currency} timezone={timezone} />
              </motion.div>
            ))}
          </div>
...
// THE REDESIGNED HORIZONTAL CARD - UPDATED WITH DYNAMIC PRICING + TIMEZONE PROP
function PremiumFoodCard({ dish, onAdd, currency = "SAR", timezone }) {
  // Logic to determine if price has changed
  const displayPrice = dish.currentPrice || dish.price;
  const isDiscounted = dish.isDynamic && Number(displayPrice) < Number(dish.price);
  const isSurged = dish.isDynamic && Number(displayPrice) > Number(dish.price);
