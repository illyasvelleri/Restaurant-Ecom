...
//   );
// }
// app/user/menu/page.js — PRO LUXURY REDESIGN 2025 (DYNAMIC CURRENCY + TIMEZONE FROM DB)
// ALL ORIGINAL LOGIC PRESERVED + TIMEZONE SUPPORT
...
export default function MenuPage() {
  ...
  const [whatsappNumber, setWhatsappNumber] = useState("");
  const [currency, setCurrency] = useState("SAR");
  const [timezone, setTimezone] = useState("Asia/Riyadh"); // ← NEW: restaurant timezone from DB
  const [loading, setLoading] = useState(true);
    ...
    const load = async () => {
      try {
        ...
        if (settingsRes.ok) {
          ...
          if (wa && wa.length >= 10) setWhatsappNumber(wa);
          setCurrency(settings.currency || "SAR");
          setTimezone(settings.timezone || "Asia/Riyadh"); // ← NEW: load correct timezone
        }
      } catch (err) {
        console.error("LOAD ERROR:", err);
        setProducts([]);
    ...
    <div className="min-h-screen">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {filteredItems.map((item) => (
            <motion.div key={item._id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              <PremiumMenuCard product={item} onAdd={addToCart} currency={currency} timezone={timezone} />
            </motion.div>
          ))}
        </div>
...
// ── Card Component ───────────────────────────────────────────────────
function PremiumMenuCard({ product, onAdd, currency, timezone }) {
  if (!product) return null;
    ...
    <div className="card-wrap">
      <div className="card-inner">
        ...
        <div className="card-image-wrap">
          ...
          {/* Badge — FIXED: only show if there's an active rule name */}
          {product.isDynamic && product.activeRuleName && (
            <div className={`card-badge ${isSale ? "badge-sale" : "badge-trend"}`}>
              {product.activeRuleName || (isSale ? "Privilege" : "Trending")}
            </div>
          )}
