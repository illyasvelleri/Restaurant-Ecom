...
//   );
// }
// components/ProfileIcon.jsx → FINAL CLEAN 2025 VERSION (WITH ROLE ISOLATION)
"use client";
...
export default function ProfileIcon() {
  ...
  // PRO LOGIC: Mask the user if they are an Admin/Staff
  // This ensures the Navbar treats them as a guest on user-facing routes
  const isActualCustomer = session?.user?.role === "user";
  const user = isActualCustomer ? session?.user : null;
          ...
          <>
            ...
            >
              {user ? (
                ...
                /* GUEST INTERFACE - Shown to Admins/Staff when on User roots */
                <div className="p-10 text-center">
                  <header className="mb-8">
                    <div className="w-12 h-12 mx-auto mb-4 rounded-2xl bg-gray-50 flex items-center justify-center text-gray-300">
                        <User size={24} />
                    ...
                    <h3 className="text-xl font-light tracking-tight text-black">Guest <span className="font-serif italic text-gray-400">Access</span></h3>
                    {/* OPTIONAL: Helpful hint for Admins */}
                    {session?.user && (
                      <p className="text-[9px] uppercase tracking-tighter text-gray-400 mt-2">Viewing as Signed-out user</p>
                    )}
                  </header>
