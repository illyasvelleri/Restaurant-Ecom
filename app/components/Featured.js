// components/FeaturedProducts.tsx → FINAL & PERFECT (SAME CART AS MENU)

"use client";

import { useState, useEffect } from 'react';
import Image from 'next/image';
import { Heart, ShoppingCart, ChevronLeft, ChevronRight, X, Send, Plus, Minus } from 'lucide-react';
...
`Please reply with your name and delivery address`
    ...
    return (
      <section className="py-16 bg-gradient-to-b from-white to-orange-50">
        <div className="container mx-auto px-6 text-center">
          <p className="text-3xl font-bold text-orange-600">Loading Popular Dishes...</p>
        </div>
      </section>
    );
  }
  ...
  return (
    <>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-xl text-gray-600">
              These are the dishes everyone is ordering today!
            </p>
          </div>
          ...
          <div className="md:hidden flex justify-end mb-8 space-x-3">
            <button onClick={() => scrollContainer('left')} className="p-3 bg-white rounded-full shadow-xl hover:shadow-2xl transition">
              <ChevronLeft size={28} />
            </button>
            <button onClick={() => scrollContainer('right')} className="p-3 bg-white rounded-full shadow-xl hover:shadow-2xl transition">
          ...
          <div 
            id="featured-scroll"
            className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 overflow-x-auto md:overflow-visible scroll-smooth pb-6 md:pb-0"
            ...
            {products.map((item) => (
              <div key={item._id} className="bg-white rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden group hover:-translate-y-1">
                <div className="relative h-48 bg-gradient-to-br from-orange-100 to-orange-50 flex items-center justify-center">
                  ...
                  >
                    <Heart size={18} className={favorites.includes(item._id) ? "fill-red-500 text-red-500" : "text-gray-400"} />
                  </button>
                </div>
          ...
          <div className="text-center mt-12">
            <a href="/user/popular" className="inline-flex items-center gap-3 px-8 py-4 bg-gradient-to-r from-orange-500 to-red-600 text-white font-bold text-lg rounded-full hover:shadow-2xl transition-all hover:scale-105">
      ...
      {/* EXACT SAME CART MODAL FROM MENU PAGE */}
      {showCart && (
