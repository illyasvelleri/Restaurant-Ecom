...
export default function Footer() {
  const [settings, setSettings] = useState({
    ...
    restaurantDescription: "Premium dining experience with authentic flavors.",
    logo: null,
    whatsapp: "",
    phone: "",
    email: "",
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        // ── Updated endpoint ──
        const res = await fetch("/api/restaurantDetails", {
          next: { revalidate: 60 }, // optional: revalidate every 60s (adjust as needed)
        });
        ...
        if (res.ok) {
          ...
          setSettings({
            ...
            workingHours: data.workingHours || "Daily: 11:00 AM - 11:00 PM",
          });
        }
      } catch (err) {
