...
import { useState } from "react";
import { signIn, getSession } from "next-auth/react"; // Added getSession to check role after login
import Link from "next/link";
import { useRouter } from "next/navigation";
import toast from "react-hot-toast";
import { Loader2, ChevronLeft, ArrowRight } from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (res?.error) {
      ...
      // --- LOGIC START: Redirect based on Role ---
      ...
      if (role === "admin" || role === "superadmin") {
        ...
        // Staff, Kitchen, Waiter, etc. go to their specific ID route
        router.push(`/admin/role/${userId}`);
      } else {
        // Normal customers go to the menu
        router.push("/user/menu");
      }
      // --- LOGIC END ---
    }
