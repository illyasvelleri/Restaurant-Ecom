...
// app/api/admin/settings/route.js — UPDATED: timezone support added (no lines removed)
...
export async function PUT(request) {
  try {
    ...
    // 1. Update full admin settings (timezone is now included in restaurant)
    const updatedSetting = await Setting.findOneAndUpdate(
      {},
      { $set: body },
      { upsert: true, new: true, setDefaultsOnInsert: true }
    ...
    const publicData = {
      ...
      workingHours: body?.restaurant?.workingHours || 'Daily: 11:00 AM – 11:00 PM',
      currency: body?.restaurant?.currency || 'SAR',
      timezone: body?.restaurant?.timezone || 'Asia/Riyadh', // ← NEW: sync timezone publicly
    };
