...
// app/api/user/saveOrders/route.js → FINAL FIXED 2025 (CORRECT TOTAL + ORDER ID + TIME)
// UPDATED: uses restaurant timezone from PublicSetting DB (no lines removed)

import { NextResponse } from "next/server";
import Order from "@/models/Orders";
import PublicSetting from "@/models/PublicSetting"; // ← NEW: to get timezone
import connectDB from "@/lib/db";
...
export async function POST(req) {
  ...
  try {
    ...
    // ── NEW: Fetch restaurant timezone from PublicSetting (public-safe) ──
    ...
    // Generate order time using correct restaurant timezone
    const orderTime = new Date().toLocaleString("en-GB", {
      timeZone: timezone, // ← NEW: dynamic from DB
      day: "2-digit",
      month: "short",
      year: "numeric",
      hour: "2-digit",
    ...
    const order = await Order.create({
      ...
      status: "pending",
      orderId,                          // ← saved reliably
      orderTime                         // ← saved reliably — now timezone-correct
    });
