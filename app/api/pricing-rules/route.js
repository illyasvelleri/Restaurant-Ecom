...
// api/pricing-rules — UPDATED: uses restaurant timezone from PublicSetting DB

import { NextResponse } from 'next/server';
import connectDB from '@/lib/db';
import DynamicPricingRule from '@/models/DynamicPricingRule';
import Product from '@/models/Product';
import PublicSetting from '@/models/PublicSetting'; // ← ADDED: to get timezone
import mongoose from 'mongoose';
...
export async function POST(request) {
    try {
        ...
        // ── NEW: Fetch restaurant timezone from DB (used for time-based rules) ──
        const publicSettings = await PublicSetting.findOne({}).lean() || {};
        const timezone = publicSettings.timezone || 'Asia/Riyadh';
        ...
        const rule = await DynamicPricingRule.create({
            ...
            active: active !== false,
            priority: Number(priority) || 0,
            // Optional: store timezone with rule if you want per-rule override later
            timezone: timezone,
        });
...
export async function GET(request) {
    try {
        ...
        // Fetch restaurant timezone (for frontend time display/validation)
        const publicSettings = await PublicSetting.findOne({}).lean() || {};
        const timezone = publicSettings.timezone || 'Asia/Riyadh';
        ...
        return NextResponse.json({ 
            ...
            count: rules.length,
            rules,
            timezone // ← send timezone so frontend knows how to interpret times
        });
