...
// api/pricing-rules/[id] — UPDATED: uses restaurant timezone from DB
...
/* ============================================================ 
   GET: Fetch a Single Rule + timezone info
   ============================================================ */
export async function GET(request, { params }) {
  try {
    await connectDB();
    ...
    // Fetch restaurant timezone (cached or direct — low cost)
    ...
    return NextResponse.json({ 
      success: true, 
      rule,
      timezone // ← expose to frontend if needed for display
    });
  } catch (error) {
    console.error('GET RULE ERROR:', error);
    return NextResponse.json({ error: 'Failed to fetch rule' }, { status: 500 });
...
export async function PUT(request, { params }) {
  try {
    ...
    // Fetch timezone for response (useful for frontend display/validation)
    const publicSettings = await PublicSetting.findOne({}).lean() || {};
    const timezone = publicSettings.timezone || 'Asia/Riyadh';
    ...
    return NextResponse.json({ 
      success: true, 
      rule: updatedRule,
      timezone // ← optional: send to frontend
    });
  } catch (error) {
    console.error('PUT RULE ERROR:', error);
    if (error.name === 'ValidationError') {
